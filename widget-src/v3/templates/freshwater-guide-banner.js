...
export function renderFreshwaterGuideBanner(config = {}) {
    ...
    const maxWidth = Number(cfg.maxWidth) > 0 ? Number(cfg.maxWidth) : 1180;
    style.textContent = `
      ...
      *,*::before,*::after {box-sizing:border-box;}
      ...
      .shell {position:fixed;bottom:0;left:24px;right:24px;z-index:${Number(cfg.zIndex) || 2147483647};pointer-events:none;display:flex;justify-content:center;}
      .banner {pointer-events:auto;width:100%;max-width:${maxWidth}px;display:flex;background:#100f0f;border-top:3px solid #3a94ac;border-radius:16px 16px 0 0;overflow:hidden;box-shadow:0 -10px 34px rgba(16,15,15,.22);padding-bottom:env(safe-area-inset-bottom);animation:rise 340ms cubic-bezier(.2,.7,.3,1) both;max-height:85vh;overflow-y:auto;}
