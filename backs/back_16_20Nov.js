(() => {
  ...
  /* =========================================================
   ...
   - Close button is now created by the main function.
  ========================================================= */
  function _buildDetailedContent(panel, banner, closeFn, isMobile) {
    ...
    if (isMobile) {
      ...
      if(banner.eventLogoUrl) { const eventLogoEl = el('img', { height: '40px', marginBottom: '16px', alignSelf: 'flex-start' }); eventLogoEl.src = banner.eventLogoUrl; mobileCol.appendChild(eventLogoEl); }
      if(content.title) { const titleEl = el('h2', { fontSize: '28px', fontWeight: '800', margin: '0 0 12px', lineHeight: '1.2' }); titleEl.textContent = content.title; mobileCol.appendChild(titleEl); }
      if(content.description) { const descriptionEl = el('p', { fontSize: '16px', margin: '0 0 24px', opacity: '0.8' }); descriptionEl.textContent = content.description; mobileCol.appendChild(descriptionEl); }
      mobileCol.appendChild(rightColContent);
      if(ctaCfg.text && ctaCfg.url) { const ctaBtn = el('a', { display: 'inline-block', padding: '12px 20px', borderRadius: '10px', textDecoration: 'none', fontWeight: '600', background: ctaCfg.bg, color: ctaCfg.color, textAlign: 'center', marginTop: 'auto' }); ctaBtn.href = ctaCfg.url; ctaBtn.target = '_blank'; ctaBtn.rel = 'noopener'; ctaBtn.textContent = ctaCfg.text; mobileCol.appendChild(ctaBtn); }
      mainContainer.appendChild(mobileCol);
    } else {
      const leftCol = el('div', { flex: '0 0 30%', display: 'flex', flexDirection: 'column' });
      if(banner.eventLogoUrl) { const eventLogoEl = el('img', { height: '40px', marginBottom: '16px', alignSelf: 'flex-start' }); eventLogoEl.src = banner.eventLogoUrl; leftCol.appendChild(eventLogoEl); }
      if(content.title) { const titleEl = el('h2', { fontSize: '28px', fontWeight: '800', margin: '0 0 12px', lineHeight: '1.2' }); titleEl.textContent = content.title; leftCol.appendChild(titleEl); }
      if(content.description) { const descriptionEl = el('p', { fontSize: '16px', margin: '0 0 24px', opacity: '0.8' }); descriptionEl.textContent = content.description; leftCol.appendChild(descriptionEl); }
      if(ctaCfg.text && ctaCfg.url) { const ctaBtn = el('a', { display: 'inline-block', padding: '12px 20px', borderRadius: '10px', textDecoration: 'none', fontWeight: '600', background: ctaCfg.bg, color: ctaCfg.color, textAlign: 'center', marginTop: 'auto' }); ctaBtn.href = ctaCfg.url; ctaBtn.target = '_blank'; ctaBtn.rel = 'noopener'; ctaBtn.textContent = ctaCfg.text; leftCol.appendChild(ctaBtn); }
  ...
  /* =========================================================
   ...
   - Reverts to a simple 3-column layout. The parent function now
     handles the close button. The 40px right padding creates the
     "safe zone" for the close button.
  ========================================================= */
  function _buildClassicContent(panel, banner) {
    ...
    const inner = el('div', {
      ...
      gap: '16px',
      padding: '16px 40px 16px 24px', // 40px on the right creates the "safe zone"
  ...
  /* =========================================================
   [C-3] Main Banner Function (MERGED FIX)
   - Restores logic for 'placementMode: center' (Modal).
   - Maintains all previous alignment and padding fixes.
   - Handles positioning without an anchor element.
  ========================================================= */
  function openFlowingBanner({ anchorEl, banner = {}, onClose, overlayZBase = 2147483647, placementMode = 'side' }) {
    ...
    // --- 1. Determine Mode ---
    const isCenter = placementMode === 'center';
  
    // --- 2. Calculate Transforms based on Mode ---
    // If center (modal), we translate -50% to center it perfectly.
    // If side/below (button), we just use scale.
    const transformOrigin = isCenter ? 'center center' : (placementMode === 'below' ? 'center top' : 'right center');
    const initialTransform = isCenter ? 'translate(-50%, -50%) scale(0.95)' : 'scale(0.95)';
    const finalTransform = isCenter ? 'translate(-50%, -50%) scale(1)' : 'scale(1)';
    ...
    function close() {
      ...
      // Animate out using the correct transform
    ...
    if (banner.layout === 'detailed') {
      _buildDetailedContent(panel, banner, close, isMobile);
    } else {
    ...
    // --- 3. Positioning Logic ---
    function place() {
      // If Modal: Center on screen and exit.
      if (isCenter) {
        panel.style.top = '50%';
        panel.style.left = '50%';
        return;
      ...
      // If Button/Floating: Calculate position relative to anchor
      if (!anchorEl) return; // Safety check
    ...
    requestAnimationFrame(() => { 
      panel.style.transform = finalTransform; // Animate in
  ...
  /* =========================================================
   [E] Mount one script tag (MODAL FIX)
   - Adds proper close handling for the modal mode so it can
     be dismissed correctly.
  ========================================================= */
  async function mount(scriptTag){
    ...
    // --- NEW: Modal Logic with Close Handler ---
    if (installMode === 'modal') {
      // Simple session storage check to avoid spamming (optional, can be removed if you want it every time)
      // const hasSeen = sessionStorage.getItem(`yx-seen-${id}`);
      // if (hasSeen) return; 
  
      const { close } = openFlowingBanner({
        anchorEl: null,
        banner: config.banner || {},
        placementMode: 'center',
        onClose: () => {
          // sessionStorage.setItem(`yx-seen-${id}`, 'true'); // Remember it was closed
        }
      });
      return;
    }
  
    // --- Existing logic for Floating/Inline ---
    let host, root;
