...
/* =========================================================
 [C] Banner Renderer (IMAGE SRC FIX)
 - Correctly assigns the 'src' attribute to all images in the detailed layout.
========================================================= */
/* =========================================================
...
function _buildDetailedContent(panel, banner) {
  ...
  if (banner.presentedBy) {
    ...
    pLogo.src = banner.presentedBy.logoUrl; // CORRECTED
  ...
  if (banner.eventLogoUrl) {
    const eventLogo = el('img', { height: '40px', marginBottom: '16px', alignSelf: 'flex-start' });
    eventLogo.src = banner.eventLogoUrl; // CORRECTED
    leftCol.appendChild(eventLogo);
  }
  ...
  // --- Helper for rendering scrollable sections (with CARD IMAGE FIX) ---
  const _renderScrollableSection = (items = [], type = 'card') => {
    ...
    items.forEach(item => {
      if (type === 'card' && item.imageUrl) {
        ...
        img.src = item.imageUrl; // CORRECTED
        ...
        const link = el('a', { display: 'inline-block', flexShrink: '0' }); link.href = item.url; link.target = '_blank'; link.rel = 'noopener';
        const logo = el('img', { height: '30px', opacity: '0.8' });
        logo.src = item.logoUrl; // CORRECTED
...
function _buildClassicContent(panel, banner) {
  ...
  const inner = el('div', { position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '24px', padding: '16px 24px', width: '100%', boxSizing: 'border-box' });
  ...
  const styles = el('style'); styles.textContent = `@media (max-width: 768px) { .yx-description { display: none !important; } .yx-title { font-size: 16px !important; } .yx-banner-inner { padding: 12px 16px !important; } }`;
...
function openFlowingBanner({ anchorEl, banner = {}, onClose, overlayZBase = 2147483647, placementMode = 'side' }) {
  ...
  const closeBtn = el('button', {
    ...
    color: 'inherit', fontSize: '24px', cursor: 'pointer', lineHeight: '1', zIndex: '10'
  });
